...
public class CrawlingService {
    ...
    @Transactional
    public List<LessonResponseDTO.FindResultDTO> action(String url, Long userId) {
        ...
        try {
            return LessonConverter.toCreateResultDTOList(doCrawl(driver, user));
        } catch (WebDriverException e) {
            throw new CrawlingException(ErrorStatus.CRAWLING_URL_INVALID);
        } finally {
            driver.close();
    ...
    public List<UserLesson> doCrawl(WebDriver driver, User user) {
        ...
        boolean isSet = userLessonRepository.findByUserAndAttendedSemester(user, semester)
                .isPresent();
    ...
    public UserLesson findLessonIsPresent(User user, String semester, boolean isSet, Lesson findLesson) {
        if (isSet) {
            ...
            Boolean isDup = userLessonRepository.findByUserAndAttendedSemesterAndLesson(user, semester, findLesson).isPresent();
